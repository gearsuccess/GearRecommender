...
class BiasedMF():
    ...
    def save(self):
        ...
        t.to_csv('../results/biased_mf_mu')
        t = pd.DataFrame(self.pu)
        t.to_csv('../results/biased_mf_pu')
        t = pd.DataFrame(self.qi)
        t.to_csv('../results/biased_mf_qi')
        t = pd.DataFrame(self.bu)
        t.to_csv('../results/biased_mf_bu')
        t = pd.DataFrame(self.bi)
        t.to_csv('../results/biased_mf_bi')
        t = pd.DataFrame(self.trec_output)
        t.to_csv('../results/biased_mf_trec_output', sep=' ', index=False, header=False)
    ...
    def score(self):
        ...
        self.biasedMF_logger.info(','.join(('test:', 'f1:'+str(f1), 'hit:'+str(hit), 'ndcg:'+str(ndcg), 'p:'+str(p), 'r:'+str(r), 'rmse:' + str(rmse))))
